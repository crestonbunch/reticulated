...
pub struct VarArgsList {
    // Parameters after '/' (or when '/' is omitted). Can be keyword or
    // positional arguments.
    args: Vec<PNode<VfpDef>>,
    // Parameters before '/' (See PEP 570). These are positional only arguments.
    pos_only_args: Vec<PNode<VfpDef>>,
    // *
    var_args: Option<PNode<VfpDef>>,
    // Keyword args after a * var arg
    kw_only_args: Vec<PNode<VfpDef>>,
    // **
    kw_args: Option<PNode<VfpDef>>,
...
impl ParseNode<VarArgsList> {
    ...
    fn add_tfp_def(&mut self, n: &PNode<VfpDef>) -> Result<(), &'static str> {
        match self.inner.state {
    ...
    fn add_node(&mut self, n: &NodeProxy) -> Result<(), &'static str> {
        match n {
            ...
            NodeProxy::VfpDef(n) => self.add_tfp_def(n),
            _ => Err("only Test|VfpDef allowed as child to VarArgsList"),
        }
    }
    ...
    fn add_token(&mut self, t: &Token) -> Result<(), &'static str> {
        match t {
            ...
            tok!(",") => Ok(()),
            tok!("=") => Ok(()),
            _ => Err("unexpected token parsing VarArgsList"),
        }
    }
...
impl_parse_construct!(VarArgsList, Arguments);
